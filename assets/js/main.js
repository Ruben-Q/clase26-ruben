  ...
  function hideLoadAnimation(){
    const t=document.getElementById("load");
    t&&setTimeout(()=>{t.style.display="none"},500)
  }
  function hookUpFilters()
  {
    const filterButtons =document.querySelectorAll(".content_filter .filter");
      ...
      filterButton.addEventListener("click",
        r=>{ 
          ...
          document.querySelectorAll(`.product`).forEach(i => {
            if(i.classList.contains(filter) || filter == "all")
            {
          ...
          filterButtons.forEach(s=> s.classList.remove("filter__active")),
          r.currentTarget.classList.add("filter__active")
        }
      )
    })
  };
